...
---> Added the Gains_from_File class
"""
...
class Gains_from_File(task.SingleTask):
    """Class for gains that you want to load from file. The file type must be hdf5 with a 'gain' header. The gains that are input here must be
    complex and be in the form (freq, input, time) where `input` is an array of integers running from 0 to 2*Ndish. If your gains assume
    a single complex gain for each antenna, such that your gains have inputs running up to Ndish, you can zero pad your array."""


    def process(self, data, gain_file):
        """Generate a gain timestream for the inputs and times in `data` using gains from the gain_file input.
