...
---> Added the _pointsource_bright function to generate a map with only bright point sources
...
@cli.command('pointsource_bright')
@click.option('--minflux', default=1, type=float)
@click.option('--maxflux', default=1e6, type=float)
@click.pass_context
def _pointsource_bright(ctx, minflux, maxflux):
    """Generate a point source only foreground map.

    For S > 4 Jy (at 600 MHz) use real point sources.
    """

    from cora.foreground import pointsource

    # Fetch point source maps
    ps = pointsource.RealPointSources()
    ps.nside = ctx.obj.nside
    ps.frequencies = ctx.obj.freq
    ps.flux_min = minflux
    ps.flux_max = maxflux
