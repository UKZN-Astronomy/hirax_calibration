...
class SimulateSidereal(task.SingleTask):
    ...
    def process(self, map_):
        ...
        for source_ind in range(nsource):
            ...
            vis_stream_full[source_ind,:] = vis_stream
        ...
        sstream = containers.SiderealStream_all_sources(
            source=source_map,
            freq=freqmap,
            ra=ntime,
            input=feed_index,
        ...
        sstream.vis[:] = mpiarray.MPIArray.wrap(vis_stream_full, axis=0)
        sstream.weight[:] = 1.0
...
class ExpandProducts(task.SingleTask):
    ...
    def process(self, sstream):
        ...
        new_stream = containers.SiderealStream_all_sources(prod=prod, stack=None, axes_from=sstream)
...
class MakeTimeStream(task.SingleTask):
    ...
    def process(self):
        ...
        tstream = containers.empty_timestream_all_sources(axes_from=self.sstream, time=time)
