...
m=manager.ProductManager.from_config('/home/zahra/hirax_corrcal/draco_corrcal_pipeline/prod_params.yaml')
m_nominal =manager.ProductManager.from_config('/home/zahra/hirax_corrcal/draco_corrcal_pipeline/prod_params_nominal.yaml')


tstream_brightsources = h5py.File('/home/zahra/hirax_corrcal/draco_corrcal_pipeline/draco_synthesis/tstream_brightsources_0.h5','r')


tstream_allsources = h5py.File('/home/zahra/hirax_corrcal/draco_corrcal_pipeline/draco_synthesis/4_tstream_gain_noise_0.h5','r')

rand_gains = h5py.File('/home/zahra/hirax_corrcal/draco_corrcal_pipeline/draco_synthesis/2_rand_gains_0.h5','r')
...
def mmodes(m_nominal, freq):
    ...
    covariance due to unresolved point sources, in `fit_gains_base_noiseless`.
    ...
    m_nominal: prod_params file
        ...
        getting the x and y positions of each antenna in the unperturbed array.
    freq: float64
        Observing frequency
    ...
    EW_u_coord = np.abs(EW_bl_length)/wavelength
    ellmode_nominal = EW_u_coord*2*np.pi
    mmode_unperturbed = ellmode_nominal[lims[:-1]]
...
def fit_gains_base_noiseless(m, m_nominal, tstream_allsources, tstream_allsources_with_noise = True, source_positions_unknown=True, tstream_brightsources=None, vecs_redundant = True, unity_gains = True, gains_file = None, save_gains_error = False):
    ...
    indices_unique_bls = Indices_for_reordering(m_nominal, unique, h5_file=False)

    ant1 = bl_arr_dish_indices[:,0].astype(int)
    ant2 = bl_arr_dish_indices[:,1].astype(int)

    random_gain = np.load('/home/zahra/hirax_corrcal/random_gain_1e-3_128val.npy')[:2*Ndish]
    ...
    for freq_ind in range(len(freq_array)):
        ...
        if vecs_redundant==True:
            ...
            for i in unique_m_unpert:
                ...
                point_source_cov_arranged = point_source_cov[indices_unique_bls][:,indices_unique_bls]
                m_unpert_indices = np.where(mmode_unperturbed==i)[0]
                for j in m_unpert_indices:
                    Cov_dic[j] = point_source_cov_arranged[lims[j]:lims[j+1], lims[j]:lims[j+1]]
