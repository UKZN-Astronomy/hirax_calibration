'''Update this file in driftscan/drift/core to be able to run kltransform.py'''
...
'''Added the point_source_unresolved_model function'''
...
def point_source_unresolved_model(lmax, frequencies, npol, pol_frac=1.0, pol_length=None):
    ''' Assuming the foregrounds contain point sources only - no synchrotron or polarisation'''
    fps = PointSources()

    nfreq = frequencies.size
    cv_ps = np.zeros((npol, npol, lmax + 1, nfreq, nfreq))

    cv_ps[0, 0] += skysim.clarray(fps.angular_powerspectrum, lmax, frequencies)
    return cv_ps
